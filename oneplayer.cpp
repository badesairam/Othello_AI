...
class Board : public Fl_Widget {
	...
	public:
	int player,score1,score2,heuristic,givdepth;
	int grid[8][8];
	int weight[8][8];
	char*scoreLabel;
	Fl_Box * scoreBox;
	...
	int scorecoins(int grid[][8],int player);
	int negamax(int grid[][8],int player,int alpha,int beta,int depth);
	pair<int, int> bestmove(int g[][8], int player, int depth);
};
...
int Board::scorecoins(int grid[][8],int player){
	int sum1=0,sum2=0,k=heuristic;
	for(int i=0;i<8;i++){
		for (int j = 0; j < 8; ++j)
		{
...
int Board::negamax(int grid[][8],int player,int alpha,int beta,int depth){
	//int bestscore =std::numeric_limits<int>::max();
	if(depth==0)return scorecoins(grid,player);	
	vector<pair<int,int> >moves=validmoves(grid,player);
	if(moves.size()==0){
		if(validmoves(grid,-1*player).size()==0){
			return scorecoins(grid,player);
		}
		int val = -negamax(grid,-1*player,-beta,-alpha,depth-1);
		...
		for(int i=0;i<moves.size();i++){
			...
			int val = -negamax(copygrid,-1*player,-beta,-alpha,depth-1);
			if(val >= beta){
				return val;
			}
			if(val>alpha){
...
pair<int,int> Board::bestmove(int grid[][8],int player,int depth){
...
Board::Board(int k,int y) : Fl_Widget (0,0,880,8*(tilesize+1),"Othello") {
                   player=-1;heuristic=k;givdepth=y;
				   for(int i=0;i<8;i++){
					for(int j=0;j<8;j++){
						grid[i][j]=16581375;
					}
		   ...
		   weight[5][0]=2;weight[5][1]=-1;weight[5][2]=1;weight[5][3]=0;weight[5][4]=0;weight[5][5]=1;weight[5][6]=-1;weight[5][7]=2;
		   weight[6][0]=-3;weight[6][1]=-4;weight[6][2]=-1;weight[6][3]=-1;weight[6][4]=-1;weight[6][5]=-1;weight[6][6]=-4;weight[6][7]=-3;
...
void Board::draw () { // this gets called automatically when redraw() is called
		 /*	for(int i=0;i<=7;i++){
		 		fl_draw_box(FL_FLAT_BOX,i*(tilesize+1)+12,0,tilesize,12,128);
		 		fl_draw_box(FL_FLAT_BOX,0,i*(tilesize+1)+12,12,tilesize,128);
		 		}*/
...
int main(int argc, char *argv[]) {
	...
	window->color(56);   
	int k=atoi(argv[1]);
	int givdepth=atoi(argv[2]);
	Board *b = new Board(k,givdepth);
	...
	Fl_Box *scorebox1 = new Fl_Box(608,3*(tilesize+1),272,(tilesize+1),"Restart");
